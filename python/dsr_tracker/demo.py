...
cap.open(0)
cv2.namedWindow('dsr')
cv2.setMouseCallback('dsr', mousehandle)
...
while (flag):
    ...
    bx,by,bw,bh = dsr.process_frame(grayframe, box)
    #cv2.waitKey(50)
    cv2.rectangle(frame,(bx,by),(bx+bw,by+bh), (0,0,255))
    end = time.time()
    fps = 1.0/(end-start)
